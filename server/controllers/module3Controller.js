...
export const submitSubsection = async (req, res) => {
  try {
    ...
    const ref = db.collection('module3').doc(facultyId);
    ...
    console.error('Module 3 Submit Subsection Error:', error);
...
export const getFacultySubsections = async (req, res) => {
  try {
    ...
    console.log('getFacultySubsections called for:', facultyId);
    ...
    console.log('Module3 subsections found:', subsections.length);

    return res.status(200).json({ success: true, data: subsections });
  } catch (error) {
    console.error('Module 3 Get Faculty Subsections Error:', error);
    return res.status(500).json({ success: false, message: 'Server error' });
  }
};
...
export const hodViewAllSubmissions = async (req, res) => {
  try {
    ...
    for (const f of facultyIds) {
      const doc = await db.collection('module3').doc(f.id).get();
      if (doc.exists) {
        results.push({
          facultyId: f.id,
          facultyName: f.name,
    ...
    console.error('Module 3 HOD View Submissions Error:', error);
...
export const hodVerifyCriterion = async (req, res) => {
  try {
    ...
    console.error('Module 3 HOD Verify Criterion Error:', error);
    res.status(500).json({ success: false, message: 'Server error' });
  }
};