...
export const getDepartmentReviewers = async (req, res) => {
  try {
    const principalCollege = String(req.admin?.college || "").trim();
    ...
    console.error("Get department reviewers error:", error);
    return res.status(500).json({ success: false, message: "Server error" });
  }
};

export const assignDepartmentReviewer = async (req, res) => {
  try {
    const principalCollege = String(req.admin?.college || "").trim();
    const principalUid = String(req.admin?.uid || req.admin?.id || "").trim();
    ...
    const { department, reviewerId } = req.body || {};

    if (!department || !reviewerId) {
      return res.status(400).json({
        success: false,
        message: "Department and reviewer are required",
      });
    }

    const reviewerDoc = await db
      .collection(USERS_COLLECTION)
      .doc(String(reviewerId))
      .get();
    ...
    const reviewerCollege = String(reviewerData.college || "")
      .trim()
      .toLowerCase();

    if (reviewerCollege !== principalCollege.toLowerCase()) {
      return res.status(403).json({
        success: false,
        message: "Reviewer must be from the same college",
      });
    }
    ...
    const existingSnap = await db
      ...
      .where("reviewerUid", "==", String(reviewerId))
      .get();

    if (!existingSnap.empty) {
      return res.status(409).json({
        success: false,
        message: "This reviewer is already assigned to this department",
      });
    }

    const docRef = await db.collection("departmentReviewers").add({
      college: principalCollege,
      department: String(department).trim(),
      reviewerUid: String(reviewerId),
    ...
    console.error("Assign department reviewer error:", error);
    return res.status(500).json({ success: false, message: "Server error" });
  }
};

export const removeDepartmentReviewer = async (req, res) => {
  try {
    const principalCollege = String(req.admin?.college || "")
      .trim()
      .toLowerCase();
    ...
    const docRef = db.collection("departmentReviewers").doc(id);
    ...
    if (
      principalCollege &&
      String(data.college || "").trim().toLowerCase() !== principalCollege
    ) {
    ...
    return res
      .status(200)
      .json({ success: true, message: "Reviewer removed successfully" });
  } catch (error) {
    console.error("Remove department reviewer error:", error);
    return res.status(500).json({ success: false, message: "Server error" });
  }
};
