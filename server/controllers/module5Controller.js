...
export const submitSubsection = async (req, res) => {
  try {
    ...
    const ref = db.collection('module5').doc(facultyId);
    ...
    console.error('Module 5 Submit Subsection Error:', error);
...
export const getFacultySubsections = async (req, res) => {
  try {
    ...
    console.error('Module 5 Get Faculty Subsections Error:', error);
    return res.status(500).json({ success: false, message: 'Server error' });
  }
};
...
export const hodViewAllSubmissions = async (req, res) => {
  try {
    ...
    for (const f of facultyIds) {
      const doc = await db.collection('module5').doc(f.id).get();
      if (doc.exists) {
        results.push({
          facultyId: f.id,
          facultyName: f.name,
    ...
    console.error('Module 5 HOD View Submissions Error:', error);
...
export const hodVerifyCriterion = async (req, res) => {
  try {
    ...
    console.error('Module 5 HOD Verify Criterion Error:', error);
    res.status(500).json({ success: false, message: 'Server error' });
  }
};