...
export const submitSubsection = async (req, res) => {
  try {
    ...
    const ref = db.collection("module4").doc(facultyId);
    ...
    console.error("Module 4 Submit Subsection Error:", error);
...
export const getFacultySubsections = async (req, res) => {
  try {
    ...
    console.error("Module 4 Get Faculty Subsections Error:", error);
    return res.status(500).json({ success: false, message: "Server error" });
  }
};
...
export const hodViewAllSubmissions = async (req, res) => {
  try {
    ...
    for (const f of facultyIds) {
      const doc = await db.collection("module4").doc(f.id).get();
      if (doc.exists) {
        results.push({
          facultyId: f.id,
          facultyName: f.name,
    ...
    console.error("Module 4 HOD View Submissions Error:", error);
...
export const hodVerifyCriterion = async (req, res) => {
  try {
    ...
    if (!criterion)
      return res
        .status(404)
        .json({ success: false, message: "Criterion not found" });
    ...
    console.error("Module 4 HOD Verify Criterion Error:", error);
    res.status(500).json({ success: false, message: "Server error" });
  }
};
