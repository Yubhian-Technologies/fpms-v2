...
export default function AppealReview() {
  ...
  const handleAppealReview = async (item: SubmissionItem) => {
    ...
    try {
      ...
      // Move from pending queue → resolved list, or update in-place if already resolved
  ...
  const renderAppealCard = (
    ...
    const input = reviewInputs[id] || {
      appealerScore: isEditing ? String(item.appealerScore ?? "") : "",
      appealerReason: isEditing ? (item.appealerReason ?? "") : "",
    };
