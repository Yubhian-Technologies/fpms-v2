...
export default function Dashboard() {
              ...
              {deptStats.length === 0 ? (
                ...
                <Accordion type="multiple" className="w-full">
                  {Array.from(new Set(deptStats.map((d: any) => d.college))).sort().map((college: any) => {
                    ...
                    const totalSubmittedFaculty = rows.reduce((s: number, d: any) => s + (d.submitted ?? 0), 0);
                    ...
                    const collegeReviewRate = totalSubmittedFaculty > 0 ? Math.round((totalAcceptedFaculty / totalSubmittedFaculty) * 100) : 0;
                        ...
                        <AccordionTrigger className="text-sm font-medium hover:no-underline py-3">
                          <div className="flex flex-col gap-1.5 w-full pr-3 text-left">
                            ...
                            <div className="flex items-center gap-3 text-xs text-muted-foreground font-normal flex-wrap">
                              ...
                              <span className={`font-semibold ${reviewRateColor}`}>Review {collegeReviewRate}%</span>
                              <Progress value={Math.min(collegeReviewRate, 100)} className="h-1.5 flex-1 min-w-[100px]" />
                            </div>
                          </div>
                        </AccordionTrigger>
                        <AccordionContent className="pt-1 pb-3">
                          ...
                          <div className="md:hidden space-y-2 px-1">
                            {rows.map((d: any) => {
                              ...
                              const reviewRate = (d.submitted ?? 0) > 0 ? Math.round(((d.acceptedFaculty ?? 0) / (d.submitted ?? 0)) * 100) : 0;
                              const subRateColor = subRate >= 80 ? "text-emerald-600" : subRate >= 50 ? "text-amber-600" : "text-red-500";
                              ...
                              return (
                                <div key={d.department} className="rounded-lg border bg-muted/20 p-3 space-y-2">
                                  <div className="flex items-center justify-between gap-2">
                                    ...
                                    <span className={`text-sm font-bold shrink-0 ${reviewRateColorMobile}`}>Review {reviewRate}%</span>
                                  </div>
                                  <Progress value={Math.min(reviewRate, 100)} className="h-1" />
                            ...
                            <table className="w-full min-w-[640px] text-xs">
                              <thead>
                                <tr className="bg-muted/40 border-b">
                                  ...
                                  <th className="text-center px-3 py-2 font-semibold text-muted-foreground">Review Rate</th>
                              ...
                              <tbody>
                                {rows.map((d: any) => (
                                  <tr key={d.department} className="border-b last:border-0 hover:bg-muted/20">
                                    ...
                                    <td className="px-3 py-2 text-center">
                                      {(() => {
                                        const rr = (d.submitted ?? 0) > 0 ? Math.round(((d.acceptedFaculty ?? 0) / (d.submitted ?? 0)) * 100) : 0;
                                        return (
                                          <>
                                            <span className={`font-semibold ${rr >= 75 ? "text-emerald-600" : rr >= 40 ? "text-amber-600" : "text-red-500"}`}>{rr}%</span>
                                            <div className="text-xs text-muted-foreground">{d.acceptedFaculty ?? 0}/{d.submitted ?? 0}</div>
                                          </>
                                        );
                                      })()}
                                    </td>
                                ...
                                <table className="w-full text-sm">
                                  ...
                                  <tbody>
                                    {deansV.map((s: any) => {
                                      ...
                                      const appealScoreV = subs.some((sub: any) => sub.status === "appeal-resolved") ? finalScoreV : null;
  ...
  if (
    ...
    // ── CONSOLIDATED REPORT: all faculty × all tasks matrix ──
    ...
    const buildFacultyPDF = (
      ...
      for (const dept of depts) {
        ...
        const rows = sortedStaff.map((s: any, idx: number) => {
          ...
          const claimed = subs.reduce((sum: number, sub: any) => sum + Number(sub.claimedScore ?? 0), 0);
          const reviewer = subs.reduce((sum: number, sub: any) => sum + Number(sub.reviewerScore ?? 0), 0);
          const isFinalized = subs.length > 0 && subs.every((sub: any) =>
            ["accepted", "appeal-resolved", "auto-approved", "appeal-expired"].includes(sub.status)
          );
          const finalScore = isFinalized
              ...
              <div className="flex flex-wrap items-center justify-between gap-4">
                ...
                <div className="flex gap-2 flex-wrap">
                  ...
                  <Button variant="outline" size="sm" onClick={exportConsolidatedReport}>
                    <FileSpreadsheet className="mr-2 h-4 w-4" /> Consolidated
                  </Button>
                      ...
                      <table className="w-full text-sm">
                        ...
                        <tbody>
                          {deans.map((s: any) => {
                            ...
                            const appealScore = subs.some((sub: any) => sub.status === "appeal-resolved") ? finalScore : null;
                            const pct = target && target > 0 && finalScore != null ? Math.round((finalScore / target) * 100) : null;
                            const hasAppealed = subs.some((sub: any) => sub.status === "appealed");
                            const hasPending = subs.some((sub: any) => sub.status === "submitted");
                            const hasReviewed = subs.some((sub: any) => sub.status === "reviewed");
